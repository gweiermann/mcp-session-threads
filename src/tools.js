...
export function registerTools(mcp, client) {
  ...
  mcp.tool(
    'add_message',
    'Append an agent message to a thread (Markdown). Start with a one-line summary of what the user asked. Adding a message to a resolved thread reopens it. Optionally set `intent` — pick by WHOSE turn it is next:\n' +
      '• "status" — an INTERIM, optional update where a real reply is still GUARANTEED to follow (e.g. "on it — here is the plan", "still running the tests"). It is a nice-to-have, NOT your answer and NOT anything the user must act on. The thread stays "waiting on agent" so the user is not prompted — so you MUST come back with a real reply later. Do NOT use "status" for your final answer or when you actually want the user to respond.\n' +
      '• "discussion" — you are in a back-and-forth and want the user\'s take/opinion on what you just wrote; hands the turn to the user.\n' +
      '• "question" — you need a specific answer from the user to proceed.\n' +
      '• "proposal" — you are proposing an approach for the user to approve/adjust.\n' +
      '• "done" — you finished this thread.\n' +
      'Only "status" keeps the turn on your side (all others hand it to the user); the labels are subtle UI hints, not tag badges. Optionally refresh `actions`/`tags`.',
